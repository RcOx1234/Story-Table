...
import { Plus, Heart, LayoutGrid, List, Users } from 'lucide-react';
      ...
      {viewMode === 'list' ? (
        <>
          <div className="min-w-0 flex-1 pr-2">
            <div className="flex min-w-0 items-baseline gap-2">
              ...
              {char.alias && <span className="truncate text-xs text-[#5A6078]">"{char.alias}"</span>}
            </div>
            <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5">
              ...
              {houseName(char) && <span className="text-[11px] text-[#5A6078]">{houseName(char)}</span>}
            </div>
          </div>
        ...
        <div>
          ...
          <div className="mb-2 flex flex-col items-center gap-1">
            ...
            {houseName(char) && <span className="text-xs text-[#5A6078]">{houseName(char)}</span>}
          </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <EntityFoldersSection
        ...
        emptyIcon={<Users size={48} className="mx-auto mb-4 text-[#2A3045]" />}
        emptyMessage="No hay personajes con estos filtros"
        onAddItem={openNew}
        gridClassName={
          viewMode === 'grid'
