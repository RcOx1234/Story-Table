...
export function DatosSection({ worldId }: Props) {
  ...
  const renderDatum = (datum: WorldDatum, i: number) => (
    <motion.div
      key={datum.id}
  ...
  return (
    <div>
      <EntityFoldersSection
        ...
        emptyIcon={<BookOpen size={48} className="mx-auto mb-4 text-[#2A3045]" />}
        emptyMessage="No hay datos del mundo"
        onAddItem={openNew}
