...
export function Sidebar() {
  ...
  const menuItems = [
    { icon: LayoutDashboard, label: 'Mi Mesa', path: '/', count: null },
    { icon: Globe, label: 'Mis Mundos', path: '/', count: worlds.length },
    { icon: Lightbulb, label: 'Ideas', path: '/ideas', count: ideas.length },
    { icon: Heart, label: 'Favoritos', path: '/favorites', count: null },
    { icon: Trash2, label: 'Papelera', path: '/trash', count: null },
  ];
      ...
      <div className="w-14 bg-[#111318] border-r border-[#1E2230] flex flex-col items-center py-4 flex-shrink-0">
        ...
        <div className="mt-4 space-y-2">
          {menuItems.map((item) => (
            <button
              key={item.path}
              onClick={() => navigate(item.path === '/worlds' ? '/' : item.path)}
